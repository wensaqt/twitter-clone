		...
		<>
			...
			<div className='border-b-[1px] border-neutral-800 pb-4'>
				<div className='flex justify-end p-2'>
					{userId === user._id ? (
						<Button label={'Editer profil'} secondary onClick={() => editModal.onOpen()} />
					) : user.isFollowing ? (
						<Button label={'Ne plus suivre'} outline onClick={onUnfollow} disabled={isLoading} isLoading={isLoading} />
					) : (
						<Button label={'Suivre'} onClick={onFollow} disabled={isLoading} isLoading={isLoading} />
					)}
				</div>
					...
					<div className='flex flex-col mt-4'>
						...
						<div className='flex flex-row items-center mt-6 gap-6'>
							<div className='flex flex-row items-center gap-1 hover:underline cursor-pointer' onClick={openFollowModal}>
								...
								<p className='text-neutral-500'>Abonnements</p>
							</div>
					...
					<>
						<div className='flex flex-row w-full py-3 px-4'>
							<div
								className={cn(
									...
									state === 'following' && 'border-b-[2px] border-orange-500 text-orange-500'
								)}
								onClick={onFollowing}
								role='button'
							>
							...
							<div
								className={cn(
									'w-[50%] h-full flex justify-center items-center cursor-pointer font-semibold',
									state === 'followers' && 'border-b-[2px] border-orange-500 text-orange-500'
								)}
								onClick={onFollowers}
								role='button'
							>
						...
						{isFetching ? (
							...
							<div className='flex flex-col space-y-4'>
								{state === 'following' ? (
									following.length === 0 ? (
										<div className='text-neutral-600 text-center p-6 text-xl'>Aucun abonnement</div>
									) : (
										following.map(user => <FollowUser key={user._id} user={user} setFollowing={setFollowing} />)
									)
								) : followers.length === 0 ? (
									<div className='text-neutral-600 text-center p-6 text-xl'>Pas d'abonné</div>
								) : (
